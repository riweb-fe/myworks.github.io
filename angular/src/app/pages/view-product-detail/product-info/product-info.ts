...
@Component({
  ...
  template: `
    ...
    <app-star-rating [rating]="product().rating" class="mb-3 block">
      {{ product().rating }}&ensp;({{ product().reviewCount }}レビュー)
    </app-star-rating>
    <p class="text-3xl font-extrabold mb-4">{{ product().price.toLocaleString() }}円</p>
    <app-stock-status class="mb-4" [inStock]="product().inStock" />
    <p class="font-semibold mb-2">商品概要</p>
    <p class="text-gray-600 border-b border-gray-200 pb-4">{{ product().description }}</p>

    <div class="flex items-center gap-2 mb-2 pt-4">
    ...
    <div class="flex items-center gap-4 mb border-b border-gray-200 pb-4">
      <button matButton="filled" class="w-2/3 flex items-center gap-2" (click)="store.addToCart(product(), quantity())" [disabled]="!product().inStock">
        ...
        {{ product().inStock ? 'カートに追加' : '在庫がありません' }}
      </button>
